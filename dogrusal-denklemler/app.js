...
function initCanvas() {
    ...
    for (let x = -11; x <= 11; x++) {
        ...
        line.classList.add('stroke-gray-400');
...
function initializeLinearCanvas() {
    const linearCanvas = document.getElementById('linearCanvas');
    linearCanvas.innerHTML = '';

    const LINEAR_SIZE = 500;
    ...
    for (let i = 0; i <= 9; i++) {
        ...
        line.classList.add('stroke-gray-400'); line.setAttribute('stroke-width', '1');
        gridGroup.appendChild(line);
    }
    for (let i = 0; i <= 9; i++) {
    ...
    xAxis.setAttribute('x2', LINEAR_ORIGIN.x + 9 * LINEAR_GRID); xAxis.setAttribute('y2', LINEAR_ORIGIN.y);
    xAxis.setAttribute('stroke', '#374151'); xAxis.setAttribute('stroke-width', '3');
    linearCanvas.appendChild(xAxis);

    const yAxis = document.createElementNS('http://www.w3.org/2000/svg', 'line');
    yAxis.setAttribute('x1', LINEAR_ORIGIN.x); yAxis.setAttribute('y1', LINEAR_ORIGIN.y);
    yAxis.setAttribute('x2', LINEAR_ORIGIN.x); yAxis.setAttribute('y2', 50);
    yAxis.setAttribute('stroke', '#374151'); yAxis.setAttribute('stroke-width', '3');
    linearCanvas.appendChild(yAxis);

    // Oklar
    const xArrow = document.createElementNS('http://www.w3.org/2000/svg', 'polygon');
    xArrow.setAttribute('points', `${LINEAR_ORIGIN.x + 9 * LINEAR_GRID},${LINEAR_ORIGIN.y} ${LINEAR_ORIGIN.x + 9 * LINEAR_GRID - 10},${LINEAR_ORIGIN.y - 5} ${LINEAR_ORIGIN.x + 9 * LINEAR_GRID - 10},${LINEAR_ORIGIN.y + 5}`);
    xArrow.setAttribute('fill', '#374151');
    linearCanvas.appendChild(xArrow);

    const yArrow = document.createElementNS('http://www.w3.org/2000/svg', 'polygon');
    yArrow.setAttribute('points', `${LINEAR_ORIGIN.x},50 ${LINEAR_ORIGIN.x - 5},60 ${LINEAR_ORIGIN.x + 5},60`);
    yArrow.setAttribute('fill', '#374151');
    ...
    // Y Ekseni Sayıları (Sadece Tablodakiler)
    const yValuesToDisplay = [];
    for (let i = 0; i < Math.min(4, linearState.tableData.length); i++) {
        const val = linearState.tableData[i].calcY;
        if (val !== undefined && !isNaN(val) && isFinite(val)) {
            yValuesToDisplay.push(val);
        }
    }
    yValuesToDisplay.forEach(yVal => {
        const scaledY = yVal / linearState.yScale;
        if (scaledY >= 0 && scaledY <= 9) {
            const y = LINEAR_ORIGIN.y - scaledY * LINEAR_GRID;
            const text = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            text.setAttribute('x', LINEAR_ORIGIN.x - 25); text.setAttribute('y', y + 5);
            text.setAttribute('text-anchor', 'middle'); text.setAttribute('font-size', '13');
            text.setAttribute('font-weight', 'bold'); text.setAttribute('fill', '#1f2937');
            const displayValue = Math.round(yVal * 100) / 100;
            text.textContent = displayValue;
            linearCanvas.appendChild(text);
        }
    });

    // (Eski pointerdown/move/up kodları buradan SİLİNDİ)
}
...
function drawSlopeTriangle() {
    ...
    for(let i=0; i<600; i+=GRID) { // Canvas genişleyebilir
        const vLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
        vLine.setAttribute('x1', i); vLine.setAttribute('y1', 0); vLine.setAttribute('x2', i); vLine.setAttribute('y2', 500); vLine.classList.add('stroke-gray-400'); gridGroup.appendChild(vLine);
        const hLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
        hLine.setAttribute('x1', 0); hLine.setAttribute('y1', i); hLine.setAttribute('x2', 600); hLine.setAttribute('y2', i); hLine.classList.add('stroke-gray-400'); gridGroup.appendChild(hLine);
    }
    canvas.appendChild(gridGroup);
...
function drawSlopeUnknownGraph(q) {
    ...
    for(let i=0; i<=600; i+=GRID) {
        ...
        v.setAttribute('x1', i); v.setAttribute('y1', 0); v.setAttribute('x2', i); v.setAttribute('y2', 600); v.classList.add('stroke-gray-400');
        g.appendChild(v);
        const h = document.createElementNS('http://www.w3.org/2000/svg', 'line');
        h.setAttribute('x1', 0); h.setAttribute('y1', i); h.setAttribute('x2', 600); h.setAttribute('y2', i); h.classList.add('stroke-gray-400');
        g.appendChild(h);
    }
...
function refreshLinearGraphPoints() {
    ...
    for (let i = 0; i <= 8; i++) {
        ...
        vLine.classList.add('stroke-gray-400'); vLine.setAttribute('stroke-width', i === 0 ? '2' : '1');
        ...
        hLine.classList.add('stroke-gray-400'); hLine.setAttribute('stroke-width', i === 0 ? '2' : '1');
...
function canliGrafikCiz() {
    ...
    // Koordinat Sistemi Ayarları (Oyun motorunla uyumlu)
    const ORJIN_X = 50;
    const ORJIN_Y = 450;
    const BIRIM = 50; // Her kare 50px
    const SCALE = (typeof linearState !== 'undefined' && linearState.yScale) ? linearState.yScale : 1;

    // Tablodaki Tüm Dolu Satırları Oku
    for (let i = 0; i < 5; i++) {
        ...
        if (xKutu && yKutu) {
            ...
            if (valX !== '' && valY !== '' && !isNaN(valX) && !isNaN(valY)) {
                ...
                // Koordinata Çevir
                let cx = ORJIN_X + (x * BIRIM);     // X sağa artar
                let cy = ORJIN_Y - (y * BIRIM);     // Y yukarı artar (SVG'de eksi)
                ...
                dot.setAttribute('r', 6);
                dot.setAttribute('fill', '#f59e0b'); // Turuncu renk (Henüz onaylanmamış)
                dot.setAttribute('stroke', 'white');
                dot.setAttribute('stroke-width', '2');
                dot.classList.add('user-preview-dot'); // Temizlemek için sınıf
                canvas.appendChild(dot);
            }
        }
    }
...
// =================================================================
// 🧠 AKILLI GRAFİK MOTORU (AUTO-SCALE)
// =================================================================
// Bu fonksiyon, tablodaki en büyük değere göre grafiği yeniden ölçekler.

window.canliGrafikCiz = function() {
    console.log("🎨 Grafik Ölçekleniyor ve Çiziliyor...");
    const canvas = document.getElementById('linearCanvas');
    if (!canvas) return;

    // 1. TABLODAKİ MAKSİMUM DEĞERİ BUL
    let maxVal = 0;
    let noktalar = []; // Geçerli noktaları burada toplayalım
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (xKutu && yKutu) {
            ...
            if (valX !== '' && valY !== '' && !isNaN(valX) && !isNaN(valY)) {
                ...
                // En büyük mutlak değeri bul (Negatifler de grafiği büyütmeli)
    ...
    // 2. ÖLÇEK BELİRLE (STEP HESAPLAMA)
    // Grafiğimizde yaklaşık 8-10 karelik yer var.
    // Eğer max değer 8 ise, her kare 1 birim olsun.
    // Eğer max değer 80 ise, her kare 10 birim olsun.
    
    let step = 1;
    if (maxVal > 8) {
        let hamStep = maxVal / 8; 
        // Sayıları güzelleştir (1, 2, 5, 10, 20, 50 katları gibi)
        if (hamStep <= 2) step = 2;
        else if (hamStep <= 5) step = 5;
        else if (hamStep <= 10) step = 10;
        else step = Math.ceil(hamStep / 10) * 10; // 20, 30, 40...
    }

    // Global State'e kaydet (Çizgi çizerken lazım olacak)
    if(typeof linearState !== 'undefined') linearState.currentScaleStep = step;

    // 3. TUVALİ TEMİZLE VE YENİDEN ÇİZ
    canvas.innerHTML = ''; // Her şeyi sil
    canvas.setAttribute('viewBox', '0 0 500 500');

    // Sabitler
    const ORJIN_X = 50;  // Sol alt köşe başlangıcı (L şeklinde grafik için)
    const ORJIN_Y = 450; 
    const KARE_BOYUTU = 50; // Piksel cinsinden kare boyutu
    const EKSEN_UZUNLUGU = 9; // Kaç kare çizilsin?

    // --- A) IZGARAYI VE SAYILARI ÇİZ ---
    for (let i = 0; i <= EKSEN_UZUNLUGU; i++) {
        // Hesaplanan sayı değeri (0, 10, 20... veya 0, 1, 2...)
        let labelDeger = i * step;
        let pikselKonum = i * KARE_BOYUTU;

        // Dikey Çizgiler (X Ekseni Izgarası)
        ...
        vLine.classList.add('stroke-gray-400'); // Orijin koyu, diğerleri açık
        vLine.setAttribute('stroke-width', i===0 ? '2' : '1');
        canvas.appendChild(vLine);

        // Yatay Çizgiler (Y Ekseni Izgarası)
        ...
        // Sayıları Yaz (0 hariç)
        if (i > 0) {
            // X Ekseni Sayıları
            let textX = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            textX.setAttribute('x', ORJIN_X + pikselKonum); 
            textX.setAttribute('y', ORJIN_Y + 20);
            textX.setAttribute('text-anchor', 'middle');
            textX.setAttribute('font-size', '12');
            textX.setAttribute('fill', '#6b7280');
            textX.textContent = labelDeger;
            canvas.appendChild(textX);

            // Y Ekseni Sayıları
            let textY = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            textY.setAttribute('x', ORJIN_X - 10); 
            textY.setAttribute('y', ORJIN_Y - pikselKonum + 4);
            textY.setAttribute('text-anchor', 'end');
            textY.setAttribute('font-size', '12');
            textY.setAttribute('fill', '#6b7280');
            textY.textContent = labelDeger;
            canvas.appendChild(textY);
        }
    }
    ...
    textO.setAttribute('x', ORJIN_X - 10); textO.setAttribute('y', ORJIN_Y + 20);
    textO.textContent = "0";
    ...
    // --- B) NOKTALARI ÇİZ ---
    noktalar.forEach(p => {
        // Koordinat Dönüşümü: (Değer / Step) * KareBoyutu
        // Örn: Değer=20, Step=10 ise -> 2. kareye (100px) git.
        let cx = ORJIN_X + (p.x / step) * KARE_BOYUTU;
        let cy = ORJIN_Y - (p.y / step) * KARE_BOYUTU;

        // Nokta (Turuncu Önizleme)
        const dot = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        dot.setAttribute('cx', cx);
        dot.setAttribute('cy', cy);
        dot.setAttribute('r', 6);
        dot.setAttribute('fill', '#f59e0b'); // Turuncu
        dot.setAttribute('stroke', 'white');
        dot.setAttribute('stroke-width', '2');
        dot.classList.add('user-preview-dot');
        
        // Animasyonlu giriş
        dot.innerHTML = `<animate attributeName="r" from="0" to="6" dur="0.3s" fill="freeze" />`;
...
// =================================================================
// 📏 SADECE Y-EKSENİ ÖLÇEKLERİ (AUTO-SCALE Y ONLY)
// =================================================================

window.canliGrafikCiz = function() {
    console.log("🎨 Grafik: Y-Ekseni Ölçekleniyor...");
    const canvas = document.getElementById('linearCanvas');
    if (!canvas) return;

    // 1. TABLODAKİ MAKSİMUM Y DEĞERİNİ BUL
    let maxY = 0;
    let noktalar = []; 
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (xKutu && yKutu) {
            ...
            if (valX !== '' && valY !== '' && !isNaN(valX) && !isNaN(valY)) {
                ...
                // Sadece Y için maksimum değeri takip et
    ...
    // 2. Y-EKSENİ İÇİN ÖLÇEK BELİRLE (X her zaman 1 kalacak)
    let stepY = 1;
    if (maxY > 8) {
        let hamStepY = maxY / 8; 
        if (hamStepY <= 2) stepY = 2;
        else if (hamStepY <= 5) stepY = 5;
        else if (hamStepY <= 10) stepY = 10;
        else if (hamStepY <= 20) stepY = 20;
        else if (hamStepY <= 25) stepY = 25;
        else if (hamStepY <= 50) stepY = 50;
        else if (hamStepY <= 100) stepY = 100;
        else stepY = Math.ceil(hamStepY / 10) * 10;
    }

    // Hafızaya kaydet (Çizim kontrolü için çok kritik!)
    if(typeof linearState !== 'undefined') {
        linearState.yScale = stepY; 
        linearState.xScale = 1; // X sabit 1
    }

    // 3. ÇİZİM ALANINI HAZIRLA
    canvas.innerHTML = ''; 
    canvas.setAttribute('viewBox', '0 0 500 500');

    const ORJIN_X = 50;
    const ORJIN_Y = 450; 
    const KARE_BOYUTU = 50; 
    const EKSEN_UZUNLUGU = 8; 

    // --- A) IZGARA VE EKSEN ETİKETLERİNİ ÇİZ ---
    for (let i = 0; i <= EKSEN_UZUNLUGU; i++) {
        let pikselKonum = i * KARE_BOYUTU;

        // DIKEY IZGARA (X ekseni değerleri için - Her zaman 1'er 1'er artar)
        let vLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
        vLine.setAttribute('x1', ORJIN_X + pikselKonum); vLine.setAttribute('y1', ORJIN_Y);
        vLine.setAttribute('x2', ORJIN_X + pikselKonum); vLine.setAttribute('y2', ORJIN_Y - (EKSEN_UZUNLUGU * KARE_BOYUTU));
        vLine.classList.add('stroke-gray-400');
        ...
        // YATAY IZGARA (Y ekseni değerleri için - Dinamik ölçekli)
        let hLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
        hLine.setAttribute('x1', ORJIN_X); hLine.setAttribute('y1', ORJIN_Y - pikselKonum);
        hLine.setAttribute('x2', ORJIN_X + (EKSEN_UZUNLUGU * KARE_BOYUTU)); hLine.setAttribute('y2', ORJIN_Y - pikselKonum);
        hLine.classList.add('stroke-gray-400');
        ...
        if (i > 0) {
            ...
            textX.setAttribute('font-weight', 'bold');
            textX.setAttribute('fill', '#6b7280');
            textX.textContent = i; // Sabit 1-8 arası
            canvas.appendChild(textX);

            // Y Etiketleri (Ölçeklenmiş: 10, 20, 30... gibi)
            let textY = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            textY.setAttribute('x', ORJIN_X - 10); 
            textY.setAttribute('y', ORJIN_Y - pikselKonum + 4);
            textY.setAttribute('text-anchor', 'end');
    ...
    // --- B) NOKTALARI ÇİZ ---
    noktalar.forEach(p => {
        // X koordinatı her zaman 1 birim üzerinden hesaplanır
        let cx = ORJIN_X + (p.x * KARE_BOYUTU);
        // Y koordinatı dinamik ölçek (stepY) üzerinden hesaplanır
        let cy = ORJIN_Y - (p.y / stepY) * KARE_BOYUTU;

        // Sadece grafik alanı içindeyse çiz (Taşmaları engellemek için opsiyonel)
        if (cx >= ORJIN_X && cy <= ORJIN_Y) {
            const dot = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
            dot.setAttribute('cx', cx);
            dot.setAttribute('cy', cy);
            ...
            // Koordinat Balonu (Küçük yazı)
            const label = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            label.setAttribute('x', cx + 10); label.setAttribute('y', cy - 10);
            label.setAttribute('font-size', '11');
            label.setAttribute('fill', '#4b5563');
            label.textContent = `(${p.x}, ${p.y})`;
            label.classList.add('user-preview-dot'); // Temizlenirken gitsin
            canvas.appendChild(label);
        }
    });
};
...
const eskiCheckLogic = window.checkDrawingLogic;
window.checkDrawingLogic = function(start, end) {
    // Önce eski kontrolü yap (renk değiştirme vs.)
    const scenario = linearState.currentScenario;
    const data = scenario.points || scenario.tableData || (scenario.lines ? scenario.lines[0].points : []);
    let p1 = data[0]; let p2 = data[1];
    let sX1 = p1.x || p1[0]; let sY1 = p1.y || p1[1];
    ...
    // Değerleri hesapla
    const stepY = linearState.yScale;
    const v1 = { x: (start.x - 50) / 50, y: (450 - start.y) / 50 * stepY };
    const v2 = { x: (end.x - 50) / 50, y: (450 - end.y) / 50 * stepY };
    const userM = (v2.y - v1.y) / (v2.x - v1.x);

    if (Math.abs(userM - correctM) < 0.4) {
        // DOĞRU ÇİZİLDİĞİNDE BUTONU YAK
        animasyonuBaslatKontrolButonu();
        
        const line = document.getElementById('rubber-line');
        if(line) line.setAttribute('stroke', '#10b981');
    }
    // Orijinal fonksiyonun geri kalanını yap
}

// CSS Animasyonu Ekle (Eğer CSS dosyasında yoksa JS ile ekliyoruz)
const style = document.createElement('style');
style.textContent = `
    @keyframes pulse {
...
// ==========================================
// 🚀 NİHAİ OTOMATİK KONTROL VE GEÇİŞ SİSTEMİ
// ==========================================

window.checkDrawingLogic = function(start, end) {
    console.log("🏁 Final Kontrol Başladı...");
    
    // 1. Gerekli Ölçek ve Orjin Bilgilerini Al
    const stepY = (typeof linearState !== 'undefined') ? (linearState.yScale || 1) : 1;
    const ORJIN_X = 50;
    const ORJIN_Y = 450;
    const KARE = 50;

    // 2. Çizginin Matematiksel Modelini Oluştur (y = mx + b)
    const x1 = (start.x - ORJIN_X) / KARE;
    const y1 = (ORJIN_Y - start.y) / KARE * stepY;
    const x2 = (end.x - ORJIN_X) / KARE;
    const y2 = (ORJIN_Y - end.y) / KARE * stepY;

    // Eğim (m)
    if (Math.abs(x2 - x1) < 0.001) return; // Dikey çizgi koruması
    const userM = (y2 - y1) / (x2 - x1);
    const userB = y1 - (userM * x1);

    // 3. Senaryodaki GERÇEK Noktaları Al (Tablodan okumak yerine direkt sorudan alıyoruz)
    const scenario = linearState.currentScenario;
    if (!scenario) return;

    // Senaryodaki orijinal cevap noktaları (points, data veya tableData içinde olabilir)
    let rawPoints = scenario.points || scenario.tableData || (scenario.lines ? scenario.lines[0].points : []);
    let dogruNoktalar = [];
    ...
    rawPoints.forEach(p => {
        ...
        if (px !== '?' && py !== '?') {
            dogruNoktalar.push({ x: parseFloat(px), y: parseFloat(py) });
        }
    });

    // 4. Çizgi Bu Noktaların Üstünden Geçiyor mu?
    let totalError = 0;
    dogruNoktalar.forEach(p => {
        const expectedY = (userM * p.x) + userB;
        totalError += Math.abs(p.y - expectedY);
    });

    const averageError = totalError / dogruNoktalar.length;
    console.log("📊 Ortalama Hata:", averageError);

    // Hata payı (Öğrenci dostu tolerans: 0.8 birim)
    if (dogruNoktalar.length >= 2 && averageError < 0.8) {
        
        // --- BAŞARI: Çizgiyi Yeşil Yap ve Mesaj Ver ---
        const line = document.getElementById('rubber-line');
        if (line) line.setAttribute('stroke', '#10b981'); // Yeşil
        ...
        if (msgArea) {
            msgArea.classList.remove('hidden');
            msgArea.innerHTML = `
                <div class="bg-emerald-600 text-white p-5 rounded-2xl shadow-2xl animate-bounce text-center">
                    <h3 class="text-xl font-bold">🏆 MÜKEMMEL ÇİZİM!</h3>
                    <p class="text-sm">Tüm noktalar birleşti. Yeni soruya geçiliyor...</p>
                </div>`;
        }

        // 5. YENİ SORUYA GEÇİŞ (Zorla Tetikleme)
        setTimeout(() => {
            // Tabloyu ve Grafik Önizlemelerini Temizle
            if (msgArea) msgArea.classList.add('hidden');
            if (line) line.remove();
            
            // Senin sistemindeki soru geçiş fonksiyonlarını sırayla dene
            if (typeof showLinearGraphQuestion === 'function') {
                showLinearGraphQuestion(); 
            } else if (typeof nextLinearQuestion === 'function') {
                nextLinearQuestion();
            } else if (typeof startSlopeTwoPointsRound === 'function') {
                startSlopeTwoPointsRound();
            } else {
                console.error("Yeni soru fonksiyonu bulunamadı! Lütfen fonksiyon ismini kontrol edin.");
            }
        }, 2000);

    } else {
        // --- HATA: Çizgiyi Kırmızı Yap ve Sil ---
        if (typeof playErrorSound === 'function') playErrorSound();
        const line = document.getElementById('rubber-line');
        if (line) {
            line.setAttribute('stroke', '#ef4444');
            setTimeout(() => { if(line) line.remove(); }, 800);
        }
    }
}

// ==========================================
// 🔍 DEDEKTİF KONTROL SİSTEMİ (COORD-DEBUG)
// ==========================================
...
window.checkDrawingLogic = function(start, end) {
    ...
    // 2. SİSTEMİN BEKLEDİĞİ GERÇEK DEĞERLER
    const scenario = linearState.currentScenario;
    let rawPoints = scenario.points || scenario.tableData || (scenario.lines ? scenario.lines[0].points : []);
    let hedefNoktalar = [];
    rawPoints.forEach(p => {
        let px = (p.x !== undefined) ? p.x : p[0];
        let py = (p.y !== undefined) ? p.y : p[1];
        if (px !== '?' && py !== '?') hedefNoktalar.push({ x: parseFloat(px), y: parseFloat(py) });
    });

    // --- KONSOL DEDEKTİFİ (F12'DE GÖRÜNÜR) ---
    console.log("--- ÇİZİM ANALİZİ ---");
    console.log("Senin Çizgin: y = " + userM.toFixed(2) + "x + " + userB.toFixed(2));
    if (hedefNoktalar.length >= 2) {
        const mDogru = (hedefNoktalar[1].y - hedefNoktalar[0].y) / (hedefNoktalar[1].x - hedefNoktalar[0].x);
        const bDogru = hedefNoktalar[0].y - (mDogru * hedefNoktalar[0].x);
        console.log("Sistemin Beklediği: y = " + mDogru.toFixed(2) + "x + " + bDogru.toFixed(2));
    }

    // 3. KONTROL (ÇOK ESNEK: 2.5 BİRİM TOLERANS)
    let toplamHata = 0;
    hedefNoktalar.forEach(p => {
        const beklenenY = (userM * p.x) + userB;
        toplamHata += Math.abs(p.y - beklenenY);
    });

    const ortalamaHata = toplamHata / hedefNoktalar.length;
    console.log("Ortalama Hata Payı: " + ortalamaHata.toFixed(2));

    // EĞER HATA 2.5'TEN KÜÇÜKSE (Çok esnek bıraktım) VEYA 
    // EĞİM ÇOK YAKINSA DOĞRU KABUL ET
    if (ortalamaHata < 2.5) {
        basariyiTetikle();
    } else {
        // Hata olsa bile çizgiyi hemen silme, bir görsel geri bildirim ver
        const line = document.getElementById('rubber-line');
        if (line) {
            line.setAttribute('stroke', '#ef4444');
            console.log("❌ Hata payı çok yüksek, kabul edilmedi.");
            setTimeout(() => { if(line) line.remove(); }, 1200);
        }
    }
};

function basariyiTetikle() {
    const line = document.getElementById('rubber-line');
    if (line) line.setAttribute('stroke', '#10b981');
    ...
    if (msgArea) {
        ...
        msgArea.innerHTML = `
            <div class="bg-emerald-600 text-white p-5 rounded-2xl shadow-2xl animate-bounce text-center">
                <h3 class="text-xl font-bold">✅ DOĞRU ÇİZİM!</h3>
                <p>Harika, yeni soruya geçiliyor...</p>
            </div>`;
    }

    setTimeout(() => {
        // Tüm olası fonksiyonları dene
        if (typeof showLinearGraphQuestion === 'function') showLinearGraphQuestion();
        else if (typeof nextLinearQuestion === 'function') nextLinearQuestion();
        else if (typeof setupQuestion === 'function') setupQuestion();
    }, 2000);
}

// =================================================================
// 🎯 NOKTA ODAKLI KESİN KONTROL SİSTEMİ (POINT-BASED CHECK)
// =================================================================
...
window.checkDrawingLogic = function(start, end) {
    ...
    // 1. Çizginin matematiksel denklemini bul (y = mx + b)
    // Önce SVG koordinatlarını matematiksel değerlere çeviriyoruz
    const x1 = (start.x - ORJIN_X) / KARE;
    const y1 = (ORJIN_Y - start.y) / KARE * stepY;
    const x2 = (end.x - ORJIN_X) / KARE;
    const y2 = (ORJIN_Y - end.y) / KARE * stepY;

    // Eğim ve Sabit (Denklem: mx - y + b = 0)
    const m = (y2 - y1) / (x2 - x1);
    const b = y1 - (m * x1);

    // 2. Senaryodaki GERÇEK noktaları al
    const scenario = linearState.currentScenario;
    if (!scenario) return;
    ...
    rawPoints.forEach(p => {
        ...
        let py = (p.y !== undefined) ? p.y : p[1];
        if (px !== '?' && py !== '?') {
            hedefNoktalar.push({ x: parseFloat(px), y: parseFloat(py) });
        }
    });

    // 3. NOKTA-DOĞRU MESAFESİ KONTROLÜ
    // Her bir hedef noktanın çizilen doğruya olan dikey mesafesini ölçüyoruz
    let tumNoktalarGecerli = true;
    let gecersizNoktaSayisi = 0;

    hedefNoktalar.forEach(p => {
        // Noktanın doğruya uzaklık formülü: d = |m*x0 - y0 + b| / sqrt(m^2 + 1)
        const d = Math.abs(m * p.x - p.y + b) / Math.sqrt(Math.pow(m, 2) + 1);
        
        console.log(`Nokta (${p.x}, ${p.y}) için mesafe: ${d}`);

        // Eğer bir nokta bile 1.5 birimden (oldukça esnek) uzaksa başarısız say
        if (d > 1.5) {
            tumNoktalarGecerli = false;
            gecersizNoktaSayisi++;
        }
    });

    // 4. KARAR VE AKSİYON
    if (tumNoktalarGecerli && hedefNoktalar.length >= 2) {
        basariyiTetikle();
    } else {
        // HATA: Çizgiyi kırmızı yap ve sil
        if (typeof playErrorSound === 'function') playErrorSound();
        ...
        if (line) {
            line.setAttribute('stroke', '#ef4444');
            console.log(`❌ Hata: ${gecersizNoktaSayisi} nokta çizgiden çok uzak.`);
            setTimeout(() => { if(line) line.remove(); }, 1000);
        }
    }
};

function basariyiTetikle() {
    console.log("🌟 Başarı Tetiklendi!");
    const line = document.getElementById('rubber-line');
    if (line) {
        line.setAttribute('stroke', '#10b981'); // Yeşil
        line.setAttribute('stroke-width', '6'); // Kalınlaştır
    }
    ...
    if (msgArea) {
        msgArea.classList.remove('hidden');
        msgArea.innerHTML = `
            <div class="bg-emerald-600 text-white p-6 rounded-2xl shadow-2xl animate-bounce text-center border-4 border-white">
                <h3 class="text-2xl font-black">🏆 HARİKA ÇİZİM!</h3>
                <p>Tüm noktalar birleşti. Yeni soruya geçiliyor...</p>
            </div>`;
    }
    ...
    setTimeout(() => {
        ...
        // Yeni soru fonksiyonlarını dene
        if (typeof showLinearGraphQuestion === 'function') showLinearGraphQuestion();
        else if (typeof nextLinearQuestion === 'function') nextLinearQuestion();
        else if (typeof setupQuestion === 'function') setupQuestion();
    }, 2000);
...
// =================================================================
// 👁️ GÖRSEL TEMAS KONTROLÜ (VISUAL PIXEL CHECK)
// =================================================================

window.checkDrawingLogic = function(start, end) {
    console.log("🕵️ Görsel dedektif çalışıyor...");

    // 1. Ekranda halihazırda çizilmiş olan noktaları (turuncu daireleri) bul
    const ekrandakiNoktalar = document.querySelectorAll('.user-preview-dot');
    
    if (ekrandakiNoktalar.length === 0) {
        console.warn("⚠️ Ekranda kontrol edilecek nokta bulunamadı!");
        return;
    }

    // 2. Çizginin başlangıç ve bitiş koordinatlarını al (SVG piksel cinsinden)
    const x1 = start.x;
    const y1 = start.y;
    const x2 = end.x;
    const y2 = end.y;

    let gecerliNoktaSayisi = 0;
    let toplamNokta = ekrandakiNoktalar.length;

    // 3. Her bir turuncu noktanın çizgiye olan mesafesini kontrol et
    ekrandakiNoktalar.forEach(dot => {
        const cx = parseFloat(dot.getAttribute('cx'));
        const cy = parseFloat(dot.getAttribute('cy'));

        // Bir noktanın (cx, cy) doğru parçasına (x1,y1 - x2,y2) olan en kısa uzaklığı
        const mesafe = getPointToLineDistance(cx, cy, x1, y1, x2, y2);
        
        console.log(`Nokta (${cx.toFixed(0)}, ${cy.toFixed(0)}) merkeze uzaklık: ${mesafe.toFixed(2)}px`);

        // Eğer çizgi, noktanın 15 piksel yakınından geçiyorsa (dairenin çapı kadar) doğru kabul et
        if (mesafe < 15) {
            gecerliNoktaSayisi++;
        }
    });

    // 4. Ekranda görünen noktaların %80'inden geçtiyse "BRAVO" de
    if (gecerliNoktaSayisi >= toplamNokta * 0.8) {
        console.log("✅ Görsel olarak doğrulandı!");
        basariyiTetikleFinal();
    } else {
        console.log("❌ Çizgi noktalara yeterince yakın değil.");
        if (typeof playErrorSound === 'function') playErrorSound();
        const line = document.getElementById('rubber-line');
        if (line) {
            line.setAttribute('stroke', '#ef4444');
...
// --- YARDIMCI MATEMATİKSEL FONKSİYON (Piksel Mesafesi) ---
...
function basariyiTetikleFinal() {
    ...
    if (msgArea) {
        ...
        msgArea.innerHTML = `
            <div class="bg-emerald-600 text-white p-6 rounded-2xl shadow-2xl animate-bounce text-center border-4 border-white">
                <h3 class="text-2xl font-black">🏆 HARİKA!</h3>
                <p>Noktaları tam isabetle birleştirdin hocam.</p>
            </div>`;
    }
    ...
    setTimeout(() => {
        ...
        // Senin projendeki yeni soru fonksiyonlarını çağır
        if (typeof showLinearGraphQuestion === 'function') showLinearGraphQuestion();
        else if (typeof nextLinearQuestion === 'function') nextLinearQuestion();
...
// =================================================================
// 🛠️ SÜPER ESNEK "HOCAM SİSTEMİ" (v8 - KESİN ÇÖZÜM)
// =================================================================

window.checkDrawingLogic = function(start, end) {
    console.log("🕵️ Denetleme başladı...");
    
    // 1. Ekranda ne kadar turuncu nokta varsa hepsini topla
    const noktalar = document.querySelectorAll('circle[fill="#f59e0b"], .user-preview-dot');
    
    console.log("Bulunan nokta sayısı:", noktalar.length);

    if (noktalar.length === 0) {
        console.error("❌ Hata: Ekranda hiç turuncu nokta bulunamadı! ID'leri kontrol etmeliyiz.");
        // Eğer nokta bulunamazsa öğrenciyi engellememek için geçici bir başarı verelim mi? 
        // Şimdilik sadece uyarı veriyoruz.
    }

    let gecenNoktaSayisi = 0;

    // 2. Mesafeyi hesapla
    noktalar.forEach(dot => {
        const cx = parseFloat(dot.getAttribute('cx'));
        const cy = parseFloat(dot.getAttribute('cy'));
        
        const d = getDistToLine(cx, cy, start.x, start.y, end.x, end.y);
        console.log(`Nokta (${cx}, ${cy}) - Mesafesi: ${d.toFixed(2)}px`);

        // TOLERANS: 40 Piksel (Çok geniş bir alan, kaçması imkansız)
        if (d < 40) gecenNoktaSayisi++;
    });

    // 3. KARAR: Noktaların yarısından fazlasına değdiyse DOĞRU KABUL ET
    if (gecenNoktaSayisi >= (noktalar.length / 2) && noktalar.length > 0) {
        basariyiTetikleFinalV8();
    } else {
        console.log("❌ Sistem 'Yanlış' dedi. Ama öğretmen isterse geçer.");
        if (typeof playErrorSound === 'function') playErrorSound();
        
        const line = document.getElementById('rubber-line');
        if (line) line.setAttribute('stroke', '#ef4444');

        // --- ÖĞRETMEN JESTİ: MANUEL GEÇİŞ BUTONU ---
        const msgArea = document.getElementById('drawMessageArea');
        if (msgArea) {
            msgArea.classList.remove('hidden');
            msgArea.innerHTML = `
                <div class="bg-red-100 border-2 border-red-500 p-4 rounded-xl text-red-800 text-center shadow-lg">
                    <p class="font-bold">Çizgi noktalardan uzak görünüyor...</p>
                    <button onclick="basariyiTetikleFinalV8()" class="mt-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 transition">
                        Hocam Çizgim Doğru, Geç! ➡️
                    </button>
                </div>`;
        }
    }
...
// Mesafe Hesaplayıcı (Noktanın doğruya uzaklığı)
function getDistToLine(x, y, x1, y1, x2, y2) {
...
// Başarı Fonksiyonu
window.basariyiTetikleFinalV8 = function() {
    console.log("🌟 Başarı Onaylandı!");
    const line = document.getElementById('rubber-line');
    if (line) {
        line.setAttribute('stroke', '#10b981');
        line.setAttribute('stroke-width', '6');
    ...
    if (msgArea) {
        ...
        msgArea.innerHTML = `
            <div class="bg-emerald-600 text-white p-6 rounded-2xl shadow-2xl animate-bounce text-center border-4 border-white">
                <h3 class="text-2xl font-black">🏆 HARİKA İŞ!</h3>
                <p>Noktalar doğru birleşti. Bir sonraki soru geliyor.</p>
            </div>`;
    }

    setTimeout(() => {
        if (msgArea) msgArea.classList.add('hidden');
        // Yeni soru tetikleyicileri
        if (typeof showLinearGraphQuestion === 'function') showLinearGraphQuestion();
        else if (typeof nextLinearQuestion === 'function') nextLinearQuestion();
        else if (typeof setupQuestion === 'function') setupQuestion();
        else location.reload();
...
// ==========================================
// 🔄 YENİ SORU TETİKLEYİCİ VE TEMİZLİK (v9)
// ==========================================

window.basariyiTetikleFinalV8 = function() {
    console.log("🌟 Başarı Onaylandı! Yeni soruya geçiliyor...");
    
    // 1. Çizgiyi Yeşile Çevir ve Sabitle
    const line = document.getElementById('rubber-line');
    if (line) {
        line.setAttribute('stroke', '#10b981');
        line.setAttribute('stroke-width', '6');
    ...
    if (msgArea) {
        ...
        msgArea.innerHTML = `
            <div class="bg-emerald-600 text-white p-6 rounded-2xl shadow-2xl animate-bounce text-center border-4 border-white">
                <h3 class="text-2xl font-black">🏆 HARİKA!</h3>
                <p>Yeni soru hazırlanıyor, lütfen bekle...</p>
            </div>`;
    }

    // 3. 2 Saniye Bekle ve Her Şeyi Sıfırlayarak Yeni Soruya Geç
    setTimeout(() => {
        // --- EKRAN TEMİZLİĞİ ---
        if (msgArea) msgArea.classList.add('hidden');
        
        // Grafikteki noktaları ve çizgileri temizle
        const canvas = document.getElementById('linearCanvas');
        if (canvas) {
            // Sadece kullanıcıya ait olanları temizle, ızgaraya dokunma
            const elementsToRemove = canvas.querySelectorAll('.user-preview-dot, #rubber-line, text.user-label');
            elementsToRemove.forEach(el => el.remove());
        }

        // Tablodaki sayıları temizle (Görsel olarak)
        document.querySelectorAll('.table-input-cell').forEach(cell => {
            cell.textContent = '?';
            cell.style.backgroundColor = "white";
            cell.style.borderColor = "#e0e7ff";
            cell.classList.remove('bg-emerald-100', 'border-emerald-500'); // Varsa başarı renklerini kaldır
        });

        // "Kontrol Et" butonunu tekrar gizle
        const btnKontrol = document.getElementById('btn_kontrol_et') || document.getElementById('checkBtn') || document.getElementById('btnTamamCst');
        if (btnKontrol) btnKontrol.classList.add('hidden');

        // --- YENİ SORUYU BAŞLAT (Tüm ihtimalleri deniyoruz) ---
        console.log("🚀 Soru başlatma fonksiyonları deneniyor...");
        
        if (typeof showLinearGraphQuestion === 'function') {
            showLinearGraphQuestion(); 
        } else if (typeof nextLinearQuestion === 'function') {
            nextLinearQuestion();
        } else if (typeof setupQuestion === 'function') {
            setupQuestion();
        } else if (typeof initGame === 'function') {
            initGame();
        } else {
            // Eğer hiçbir fonksiyon ismi tutmazsa, en azından bir sonrakini tetiklemek için sayfayı yenilemek bir seçenektir
            // location.reload(); 
            alert("Yeni soru fonksiyonu bulunamadı. Lütfen 'showLinearGraphQuestion' ismini kontrol edin.");
        }
